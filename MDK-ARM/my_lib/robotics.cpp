...
Matrixf<4, 1> robotics::r2angvec(Matrixf<3, 3> R) {
  ...
  if (theta == 0 || theta == PI) {
    float angvec[4] = {//返回包含转轴的三轴分量以及转角
...
Matrixf<4, 1> robotics::r2quat(Matrixf<3, 3> R) {
  float q[4] = {
      0.5f * sqrtf(math::limit(R.trace(), -1, 1) + 1),  // q0=cos(θ/2)实部
      math::sign(R[2][1] - R[1][2]) * 0.5f *
          sqrtf(math::limit(R[0][0] - R[1][1] - R[2][2], -1, 1) +
                1),  // q1=rx*sin(θ/2)    X轴分量
      math::sign(R[0][2] - R[2][0]) * 0.5f *
          sqrtf(math::limit(-R[0][0] + R[1][1] - R[2][2], -1, 1) +
                1),  // q2=ry*sin(θ/2)    Y轴分量
      math::sign(R[1][0] - R[0][1]) * 0.5f *
          sqrtf(math::limit(-R[0][0] - R[1][1] + R[2][2], -1, 1) +
                1),  // q3=rz*sin(θ/2)    Z轴分量
  };
  return (Matrixf<4, 1>(q) / Matrixf<4, 1>(q).norm());//除以自身的范数以归一化四元数
}
//四元数转换成旋转矩阵
...
Matrixf<4, 4> robotics::invT(Matrixf<4, 4> T) {
  Matrixf<3, 3> RT = t2r(T).trans();//转置旋转矩阵(逆变换)
  Matrixf<3, 1> p_ = -1.0f * RT * t2p(T);//平移向量取反,并与转置后的旋转矩阵相乘,求得逆变换的平移
